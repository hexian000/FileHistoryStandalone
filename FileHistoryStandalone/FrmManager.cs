	...
	public partial class FrmManager : Form
	{
			...
			if (Program.CommandLine.Length > 0)
			{
				foreach (var i in Program.CommandLine)
				{
					if (i.StartsWith("--debug:"))
						Program.InitDebugLog(i.Substring(8));
					else if (i == "--hide") Hide();
		...
		private class IconHelper
		{
			...
			public static Icon GetDirectoryIcon()
			{
				...
				IntPtr _IconIntPtr = NativeMethods.SHGetFileInfo(@"", 0, ref _SHFILEINFO, (uint)Marshal.SizeOf(_SHFILEINFO), (uint)(SHGFI.SHGFI_ICON | SHGFI.SHGFI_LARGEICON));
				if (_IconIntPtr.Equals(IntPtr.Zero)) return null;
				Icon _Icon = Icon.FromHandle(_SHFILEINFO.hIcon);
				return _Icon;
			}
		}
		private Icon DirectoryIcon = IconHelper.GetDirectoryIcon();
		...
		private void 排除文件EToolStripMenuItem_Click(object sender, EventArgs e)
		{
			var exclude = new FrmExclude();
			exclude.ShowDialog(this);
		}
	}
}
